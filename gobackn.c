...
queue_t* sentPackagesAB = NULL; // JANELA
queue_t* sentPackagesBA = NULL;
queue_t* toDoPackages = NULL;
...
void A_output(msg_t message)
{
    ...
    if(queue_size(sentPackagesAB) >= 8)
        resendSentPackages();
    else
    {
...
void A_input(pkt_t packet)
{
    ...
    if(packet.seqnum == ACKNUM && packet.acknum == acknum_global)
    {
        pkt_t* aux = findSentAB(packet.acknum);
        pkt_t* acked_package;

        if( aux->acknum == ((pkt_t*)(sentPackagesAB))->acknum )
            acked_package = (pkt_t*)queue_remove(&sentPackagesAB, sentPackagesAB);
        else
            perror("UNEXPECTED ERROR -- STOP THE PROGRAM!!!!!\n\n\n\n");

        stoptimer(0);
        seqnum_global = acknum_global;

        if(sentPackagesAB)
            acknum_global = ((pkt_t*)(sentPackagesAB))->acknum;
    }
    else
    {
        A_timerinterrupt();
...
B_input(packet) struct pkt packet;
{
    ...
    if(generateChecksum(packet) == packet.checksum && packet.acknum == acknum_global)
    {
